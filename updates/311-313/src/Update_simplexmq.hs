...
import qualified NextVersion.Simplex.Messaging.Server.MsgStore.STM as NextVersion
...
import qualified PreviousVersion.Simplex.Messaging.Server.MsgStore.STM as PreviousVersion
...
instance
  ...
  transform (PreviousVersion.NEW k1 k2 k3) = do
    k1' <- transform k1
    k2' <- transform k2
    k3' <- transform k3
    return $ NextVersion.NEW <$> k1' <*> k2' <*> k3'
...
instance Transformable PreviousVersion.Message NextVersion.Message where
  transform :: PreviousVersion.Message -> IO (Maybe NextVersion.Message)
  transform (PreviousVersion.Message mId ts flags body) = do
    mId' <- transform mId
    ts' <- transform ts
    flags' <- transform flags
    body' <- transform body
    return $ NextVersion.Message <$> mId' <*> ts' <*> flags' <*> body'

correct :: Int -> [NextVersion.Message] -> [NextVersion.Message]
correct quota xs = ys <> correctedZs
  where
    (ys, zs) = splitAt quota xs
    correctedZs =
      map
        ( \msg -> case msg of
            NextVersion.Message {..} ->
              NextVersion.MessageQuota msgId msgTs
            NextVersion.MessageQuota _ _ -> msg
        )
        zs

instance Transformable PreviousVersion.MsgQueue NextVersion.MsgQueue where
  transform :: PreviousVersion.MsgQueue -> IO (Maybe NextVersion.MsgQueue)
  transform (PreviousVersion.MsgQueue queue) = do
    sizeInt <-
      fromInteger . naturalToInteger <$> atomically (lengthTBQueue queue)
    let quota = 128
    queue' <-
      atomically (flushTBQueue queue) >>= transform >>= \case
        Nothing -> return Nothing
        Just (xs :: [b]) -> atomically $ do
          let correctedXs =
                if sizeInt >= quota then take quota (correct quota xs) else xs
          q <- newTQueue
          forM_ correctedXs (writeTQueue q)
          return $ Just q
    canWrite <- newTVarIO True
    size <- newTVarIO sizeInt
    return $
      NextVersion.MsgQueue
        <$> queue'
        <*> Just quota
        <*> Just canWrite
        <*> Just size

instance Transformable Natural Int where
  transform :: Natural -> IO (Maybe Int)
  transform = return . Just . fromInteger . naturalToInteger
