...
instance
  ...
  transform (PreviousVersion.NEW k1 k2) = do
    k1' <- transform k1
    k2' <- transform k2
    return $ NextVersion.NEW <$> k1' <*> k2' <*> Just Nothing
...
-- data PreviousVersion.ServerConfig = ServerConfig
...
-- data NextVersion.ServerConfig = ServerConfig
--   { transports :: [(ServiceName, ATransport)],
--     tbqSize :: Natural,
--     serverTbqSize :: Natural,
--     msgQueueQuota :: Natural,
...
instance
  Transformable
    PreviousVersion.ServerConfig
    NextVersion.ServerConfig
  where
  transform ::
    PreviousVersion.ServerConfig -> IO (Maybe NextVersion.ServerConfig)
  transform PreviousVersion.ServerConfig {..} = do
    transports' <- transform transports
    messageExpiration' <- transform messageExpiration
    inactiveClientExpiration' <- transform inactiveClientExpiration
    smpServerVRange' <- transform smpServerVRange
    return $
      NextVersion.ServerConfig
        <$> transports'
        <*> Just tbqSize
        ...
        <*> Just logStatsInterval
        <*> Just logStatsStartTime
        <*> Just serverStatsLogFile
        <*> Just serverStatsBackupFile
        <*> Just caCertificateFile
        <*> Just privateKeyFile
        <*> Just certificateFile
        <*> smpServerVRange'
